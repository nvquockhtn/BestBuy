...
public class ProductDaoImpl extends DaoSupport implements ProductDao{
	...
	public int searchAdminProductBy(String nameProduct,int idtypemanufacturer,int idtypeproduct, int pricefrom, int pricend, int idtypestate,int idpromotion, int page, int numberinpage) 
	{
		...
		try
		{
	        ...
	        if(idpromotion!=-1)
	        {
	        	hql = hql + " and s.promotion.id = " + idpromotion;
	        }
	        int n = (page-1)*numberinpage;
	        int m = numberinpage;
	        Query query = sessionFactory.getCurrentSession().createQuery(hql);
	...
	public ArrayList<Product> getProductByIdPromotion(int idPromotion) {
		String hql = "from Product a left join fetch a.promotion where a.promotion.id=" + idPromotion;
        Query query = sessionFactory.getCurrentSession().createQuery(hql);
        ArrayList<Product> ds = (ArrayList<Product>)query.list();
        for(int i=0;i<ds.size();i++)
        {
        	Hibernate.initialize(ds.get(i).getPromotion());
        }
        return ds;
	}
